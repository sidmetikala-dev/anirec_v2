...
class BayesianRidgeRecommender:
    ...
    def _get_picture_link(self, anime_id):
        if self.anime_data is None:
            return None

        anime = self.anime_data.get(str(anime_id), {})
        main_picture = anime.get("main_picture") or {}
        return main_picture.get("medium")
    ...
    def get_recs(self, top_k=5, filter_prequels=True):
        ...
        recs["picture_link"] = recs["anime_id"].apply(
            self._get_picture_link
        )
